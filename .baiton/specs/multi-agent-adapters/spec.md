...
- [planning] T07 Extend executable resolution to resolve one executable per distinct agent id referenced in config.roles and gate stage dispatch per role instead of with one global canDispatch flag (after T01, T06; files: src/activation/executable.ts, src/extension.ts)
